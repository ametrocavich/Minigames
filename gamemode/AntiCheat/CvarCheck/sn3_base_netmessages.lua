...
NET_MESSAGES = {
	...
	SVC = {
		...
		--[[[ svc_CreateStringTable ] = { -- 12
			...
			ReadFromBuffer = function( read )
				tableName = read:ReadString()
				maxentries = read:ReadWord()
				entries = read:ReadUBitLong( math.log2( maxentries ) + 1 )
				bits = read:ReadUBitLong( 20 )
				userdata = read:ReadOneBit()
				
				if ( userdata == 1 ) then
					userdatafixedsize = read:ReadUBitLong( 12 )
					userdatanetworkbits = read:ReadUBitLong( 4 )
				end
				
				compressed = read:ReadOneBit()
				data = read:ReadBits( bits )				
			end,
			
			WriteToBuffer = function( write )
				...
				if ( userdata == 1 ) then
					write:WriteUBitLong( userdatafixedsize, 12 )
					write:WriteUBitLong( userdatanetworkbits, 4 )
				end
			...
			Cleanup = function()
				...
				bits = nil
				userdata = nil
				userdatafixedsize = nil
				userdatanetworkbits = nil
				compressed = nil
				data = nil	
			end,
