...
resource.AddFile("materials/tags/mgluatag.vtf")
resource.AddFile("materials/tags/mgluatag.vmt")
...
CreateClientConVar("MG_Thirdperson", "0", true, true)


hook.Add("CalcView",function(ply, pos, angles, fov)
	if GetConVarNumber("MG_Thirdperson") == 1 and LocalPlayer():Alive() then
		local view = {}
		view.origin = pos-(angles:Forward()*130)
		view.angles = angles
		view.fov = fov
		return view
	else
...
hook.Add("ShouldDrawLocalPlayer",function()
	if GetConVarNumber("MG_Thirdperson") == 1 then
		return true
	end
end)
